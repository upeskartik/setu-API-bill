...
router.post('/fetchBill', auth, (req, res) => {
    ...
    User.findOne({ mobileNumber: req.body.mobileNumber }, (err, data) => {
        ...
        if (err)
            return res.status(500).send({ status: "ERROR", errorCode: "unhandled-error" }); //need to add message invalid-api-parameter
        let response = {
            "status": "SUCCESS",
            "data": {
...
function checkTransaction(req, res, next) {
    //check for all the required parmeters
    if (!req.body.transaction ||!req.body.refID || !req.body.transaction.id || !req.body.transaction.amountPaid ||
        !req.body.transaction.date)  
        return res.status(400).send({ status: "ERROR", errorCode: "invalid-api-parameters" });
    //check for input type string    
    if (typeof req.body.transaction.id != "string" ||
        typeof req.body.transaction.date != "string" ||
        typeof req.body.transaction.amountPaid != "string" ||
